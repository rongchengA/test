...
function queryLiftStatusCount(){
	$.ajax({
		...
		success : function(result) {
			...
			var nameData =["停用","检修","异常","正常"];
			var nameColorData =["white","white","white","white"];
			var colorData =['#3E9CFE','#6EBEFF','#91D4FF','#2383FE'];
			var valueData =[GetPercent(stopCount,totalCount),GetPercent(checkCount,totalCount),GetPercent(warningCount,totalCount),GetPercent(noramlCount,totalCount)];
			var data = new Array();
			for (var i = 0; i < 4; i++) {
...
function getLeftTop(dataArr){
	...
	for (var i = 0; i < dataArr.length; i++) { 
		...
		if(dataArr[i].value!=0){
			text = dataArr[i].name + dataArr[i].value * 100 + '%'; 
		}		
		//4.2文字坐标		
		var textX,textY; 
			switch(i){
	    ...
	    $.ajax({
	    	...
	    	success : function(data) {
	    		...
	    		if(result!=null){
	        		...
	        		console.log(objR);
	        		console.log(result[0].region_scope_city);
	        		console.log(cityMap[result[0].region_scope_city]);
