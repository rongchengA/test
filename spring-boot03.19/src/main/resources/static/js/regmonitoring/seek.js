...
var TableInit = function() {	
	...
	oTableInit.Init = function() {
		$("#table_emergencywarn").bootstrapTable(
						{
							url : "queryEmergencyWarnList.do", // 请求后台的URL（*）
							method : "post", // 请求方式（*）
		...
		$.contextMenu({
		    ...
		    selector: "#table_emergencywarn tr",
	...
	oTableInit.queryParams = function(params) {
		...
		var vEquipAddress = $('#search_emergency_number').val();
...
function searchEmergencyRecordDetail() {
	var selectRows = $("#table_emergencywarn").bootstrapTable("getSelections");
	if(selectRows==""){
		bootbox.alert({
		    message: "请勾选后在操作!",
		    buttons: {
...
//查询
function refresh(){
	 $('#table_emergencywarn').bootstrapTable('refreshOptions',{pageNumber:1});
}
//重置查询
function doReset() {
	$("#search_equipment_code").val('');
	$("#search_emergency_number").val('');
}
//刷新弹框界面
