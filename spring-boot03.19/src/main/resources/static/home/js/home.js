        ...
        loadMenu: function (data) {
            ...
            $.each(data, function (i) {
                ...
                if (row.f_ParentId == "0") {
                    ...
                    _html += '<a >';
                    _html += '<i class="' + row.f_Icon + '"></i>' + row.f_FullName + '</span><i class="fa fa-angle-left pull-right"></i>';
                    _html += '</a>';
                    ...
                    if (childNodes.length > 0) {
                        ...
                        $.each(childNodes, function (i) {
                            ...
                            if (subchildNodes.length > 0) {
                                ...
                                _html += '<a class="menuItem" data-id="' + subrow.f_ModuleId + '" href="' + subrow.f_UrlAddress + '"> <img class="icon" src="img/Icon/' + subrow.f_Icon + '" alt="Smiley face" width="42" height="42" style="display:block"> <img class="iconblue" src="img/icon_blue/'+subrow.f_Icon+'" alt="Smiley face" width="42" height="42" style="display:none">' + subrow.f_FullName + '</a>';
                            }
                            _html += '</li>';
                        });
                        _html += '</ul>';
            ...
            /*$("#sidebar-menu li a").hover(function () {
                var d = $(this), e = d.next();
                d.find(".iconblue").css({"display":"block"});
                d.find(".icon").css({"display":"none"});
                d.parent().siblings().find(".icon").css({"display":"block"});
                d.parent().siblings().find(".iconblue").css({"display":"none"});
            })*/
            $("#sidebar-menu li a").click(function () {
                ...
                d.parent().siblings().find(".iconblue").css({"display":"none"});
                d.css({"color":"#328AF6"});
                d.parent().siblings().find("a").css({"color":"#333"});
...
//改变图标颜色
function  changeIcon(){
	$(".navs .sidebar-menu li a img").each(function (){
...
    if(confirm("当前ie版本过低请升级或使用其他浏览器, 点击确定将退出"))
    {//如果是true ，那么就把页面转向thcjp.cnblogs.com
        location.href="login.jpg";
    }
    else
    {//否则说明下了，
        alert("当前浏览器版本下会出现兼容问题");
    }
}
 ...
 * 保存用户名密码
 */
function savePswChange(){
	var psw_new = $('#new_psw').val();
	var psw_gagin = $('#new_psw_again').val();
	var psw_old =  $('#psw_old').val();
	if(psw_old == null || psw_old == ''){
		toastr.error("旧密码不能为空");
		 return;
	}
	if(psw_new == null || psw_new == ''){
		toastr.error("新密码不能为空");
		 return;
	}
	if(psw_gagin == null || psw_gagin == ''){
		toastr.error("确认密码不能为空");
		return;
	}
	if(psw_gagin != psw_new){
		toastr.error("两次新密码不一致，请重新输入");
		return;
	}
	if(psw_new.length<6||psw_new.length>16 ){
		toastr.error("新密码必须为6-16字符");
		return; 
	}
	$.ajax({
		type : "post", 
...
function  xuanxka(dataUrl,menuName,dataId){
	 $.learuntab.addTab2(dataUrl,menuName,dataId);
}